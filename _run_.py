...
noise_param = 1
noise_param = 0.3326879
args.epsilon = 0.03326879
# generate graph
graphs = []
...
acmodel = torch.load('model_test/sample_6500.pth')
...
try:
    ...
    for ep in range(1101,NUM_EP):
        ...
        for stps in range(budget):
            ...
            acmodel.replay.add(s, actual_action_embed, r1, s1, s_embs[get_action(s1, s_embs, env.possible_actions)[0]],
                       actual_action, td=np.abs(td))

            if (ep == 0 and stps < 2) or acmodel.replay.size > batch_size:
        ...
        total_loss.append(acmodel.loss_critic.detach().numpy())
