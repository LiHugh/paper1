...
import random, io
# from cher_dqn2 import DQNTrainer
from dqn_version2 import DQNTrainer
...
def arg_parse():
    ...
    parser.add_argument('--logfile', dest='logfile', type=str, default='cher_delete_final_Version_test.log',
                        help='Logging file')
    ...
    parser.add_argument('--sample_times_env', dest='times_mean_env', type=int, default=1,
                        help='Number of times to sample objective from fluence algorithm for env rewards')
    parser.add_argument('--save_model', dest='save_model', type=str, default='delete_loss',
        ...
        for ep in range(NUM_EP):
            ...
            if heuristic:
                for i in range(2):
                    ...
                    if tmp_goal > final_goal:
                        final_goal = tmp_goal
                        final_goal = initial_goal = int(final_goal)
            # final_goal = opt
            if change_baseline:
                change = Change(g, budget=budget * 2, seeds=e_seeds)
                obj1, _, _ = change()
                final_goal = initial_goal = int(obj1)
            if domain:
                # if num_nodes<20:
                ...
                # else:
                #     final_goal = initial_goal = num_nodes + len(g)/(5*num_nodes)
                # final_goal = initial_goal = len(g.edges)/(len(g)) *100/9 * num_nodes
                final_goal = initial_goal = num_nodes * 100 / (9*5)
            # t0 = len(t)
            ...
            for stps in range(budget):
                ...
                if stps == (budget - 1) or len(env.possible_actions) == 0:
                    ...
                    r = num_infl - final_goal
                    if write:
                        writer.add_scalar('Proximity',r, ep + 1)
                    if r <0:
                        r = -1
            ...
            print('Episode: ' + str(ep) + '   Initial goal:  ' + str(initial_goal) +  '   Number of n_g achieved: ' + str(num_infl))
            ...
            if ep % save_every == 0:
                torch.save(acmodel, 'delete/' + args.save_model + str(ep) + '.pth')
