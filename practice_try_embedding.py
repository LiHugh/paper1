...
def arg_parse():
    ...
    parser.add_argument('--actiondim', dest='action_dim', type=int, default=10,
...
try:
    for ep in range(3):
        ...
        for stps in range(1):
            ...
            print('##############################state_embeds:', state_embed)
            if (t0==13):
                state_embed1 = state_embed
            elif (t0==14):
                state_embed2 = state_embed
            else:
                state_embed3 = state_embed
...
print('state_embeds for garph1(13 nodes in graph) is:', state_embed1)
print('state_embeds for garph2(14 nodes in graph) is:',  state_embed2)
print('state_embeds for garph3(14 nodes in graph) is:',  state_embed3)
# print(np.dot(state_embed1, state_embed2) / (norm(state_embed1) * norm(state_embed2)))
# import numpy as np
# from numpy.linalg import norm
# a = [-0.08936434,  0.22605906,  0.14230905 , 0.11900046 , 0.5774632 , -0.02494216,
#    0.12088977 , 0.3434711,  -0.5662231  , 0.34121376]
# b = [-0.07455876 , 0.2134431   ,0.08821841,  0.11819921 , 0.5598593,  -0.25321922,
#    0.23664105 , 0.35811704, -0.4122302 ,  0.4203747 ]
# state_embed1 = [[ 0.3260345 , -0.0019934  ,-0.5273595  , 0.36756843 , 0.05899882,  0.16948877,
#   -0.1022503 ,  0.04993935 ,-0.31664267,  0.5802685 ]]
# state_embed2 = [[ 0.4568168,   0.16753086 -0.484344 ,   0.39385483, -0.07864148,  0.12067487,
#    0.10781099 , 0.24507545 ,-0.24158713 , 0.5047401 ]]
for i,j,k in zip(state_embed1,state_embed2,state_embed3):
    a=i
    b=j
    c=k


print('dot product of graph1 and graph2:', np.dot(a,b)/(norm(a)*norm(b)))
print('dot product of graph1 and graph3:', np.dot(a,c)/(norm(a)*norm(c)))
print('dot product of graph2 and graph3:', np.dot(b,c)/(norm(b)*norm(c)))