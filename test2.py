...
from practice_net_env import NetworkEnv
...
from influence_copy import influence
import pickle, os
import influence_copy as infl
import gc
...
acmodel = torch.load('model_test/sample_5000.pth')
syn = False
ratio = 5
...
def arg_parse():
    ...
    parser.add_argument('--logfile', dest='logfile', type=str, default='test_RL_5000.log',
                        help='Logging file')
    ...
    parser.add_argument('--write', dest='write', type=int, default=0,
    ...
    parser.add_argument('--eps', dest='num_ep', type=int, default=5000,
                        help='Number of Episodes')
    ...
    parser.add_argument('--min_goal', dest='min_goal', type=int, default=100,
                        help='Minimum desired goal')
    parser.add_argument('--mid_goal', dest='mid_goal', type=int, default=200,
                        help='Middle desired goal')
    parser.add_argument('--max_goal', dest='max_goal', type=int, default=300,
...
# print(len(node_attrs[0, ]))
# print(len(node_attrs[ :,1]))
changeSeeds = 1
const_features = 0
...
args.epsilon = 0.01
seeds_list = [[1289, 527, 1344, 1180, 2009], [733, 1520, 1949, 2042, 1032], [1097, 2304, 2538, 40, 2550], [2419, 1552, 356, 239, 2443], [2102, 574, 1224, 974, 1713], [77, 13, 2803, 409, 1406], [653, 1366, 2718, 89, 2700], [2591, 2873, 1828, 283, 93], [77, 2454, 2967, 2654, 2808], [395, 1692, 1240, 2063, 630], [1095, 1419, 1681, 1454, 1159], [2763, 796, 2849, 1492, 2810], [2520, 44, 2693, 2052, 2119], [1880, 1842, 1388, 2338, 360], [2831, 2904, 2703, 2182, 1270], [2645, 2794, 2710, 1547, 2672], [780, 2780, 480, 62, 1526], [2233, 1615, 2412, 232, 2797], [2361, 747, 2913, 2866, 2605], [1960, 2761, 2688, 914, 2583], [1539, 2845, 1362, 1648, 1103], [785, 690, 921, 1390, 1258], [2833, 2512, 2138, 268, 1522], [286, 401, 1020, 2176, 671], [1381, 201, 1984, 1780, 380], [2470, 2814, 534, 446, 355], [923, 1106, 2110, 1613, 506], [1387, 1072, 1397, 2013, 1773], [459, 2033, 1651, 917, 1953], [2655, 2784, 1765, 680, 346], [1607, 2696, 2376, 412, 1279], [877, 1147, 985, 2254, 1237], [1037, 2977, 2564, 2775, 1374], [2588, 551, 347, 103, 777], [1287, 150, 2080, 1545, 1204], [3020, 1732, 2206, 1478, 1006], [376, 2378, 2185, 2306, 1530], [2033, 3046, 627, 653, 436], [645, 2269, 2031, 340, 2992], [1834, 674, 2216, 46, 1952], [1527, 350, 1210, 2861, 2066], [1458, 1580, 2367, 875, 2765], [2971, 595, 455, 2520, 1127], [655, 55, 1926, 1842, 2048], [2604, 2372, 1294, 2541, 117], [1412, 902, 1225, 368, 2020], [2854, 2452, 1929, 552, 656], [2379, 314, 209, 1575, 2609], [127, 2443, 1230, 2410, 2895], [453, 432, 1340, 2874, 1384]]
    ...
    for epoch in range(1):
        for ep in range(30):
            ...
            for stps in range(budget):
                ...
                _, r, d, _, num_infl = env.step(actual_action)
            ...
            print('Reward:', tot_r)
            print('Chosen:', res, '\n')
            print('final influenced number of nodes:',num_infl)
            # logging.info('Chosen:' + str(res))
            # logging.info('Episode:' + str(ep) + 'Env Reward:' + str(r1))
            # logging.info('Episode: ' + str(ep) + ' Reward: ' + str(tot_r))
            # logging.debug('Critic Loss: ' + str(acmodel.loss_critic))
            # logging.info('Critic Loss: ' + str(acmodel.loss_critic))
            rws.append(tot_r)
            print('###############################################final influenced number of nodes:', num_infl)
