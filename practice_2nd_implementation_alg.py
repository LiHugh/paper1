...
class Buffer:
    ...
    def sample_(self, ct):
        ...
        emb_s1 = [x[6] for x in batch]
        g_dif = [x[7] for x in batch]

        return s, a, r, s1, a1, ano

    def add(self, s, a, r, s1, a1=None, ano=None, emb_s1=None, g_dif=None):
        # arr = [s, a, r, s1, a1, ano]
        arr = [s, a, r, s1, a1, ano, emb_s1, g_dif]
...
class PriortizedReplay(Buffer):
    ...
    def add(self, s, a, r, s1, a1=None, ano=None, emb_s1=None, achi_g=None, g_dif=None, td=0):
        arr = [s, a, r, s1, a1, ano, achi_g, g_dif, emb_s1]
    ...
    def sample_(self, ct):
        ...
        idx = [self.rg.choice(self.size, p=1/self.size) for _ in range(ct)]
        ...
        achi_g = [self.buffer[i][6] for i in idx]
        g_dif = [self.buffer[i][7] for i in idx]
        emb_s1 = [self.buffer[i][8] for i in idx]

        return s, a, r, s1, a1, ano

    def sample_batch(self, ct):
        ct = min(ct, self.size)
        # print('ct is ***************************', ct)
        probs = np.array(self.probs)
        probs = probs.argsort() + 1
        ...
        probs = probs ** self.beta
        probs = probs / probs.sum()
        batch = [self.rg.choice(self.size, p=probs) for _ in range(ct)]
        # batch = random.sample(self.buffer, ct)
        # s = [x[0] for x in batch]
        # a = [x[1] for x in batch]
        # r = [x[2] for x in batch]
        ...
        # a1 = [x[4] for x in batch]
        # ano = [x[5] for x in batch]
        # achi_g = [x[6] for x in batch]
        # g_dif = [x[7] for x in batch]
        # emb_s1 = [x[8] for x in batch]
        s = [self.buffer[i][0] for i in batch]
        a = [self.buffer[i][1] for i in batch]
        r = [self.buffer[i][2] for i in batch]
        s1 = [self.buffer[i][3] for i in batch]
        a1 = [self.buffer[i][4] for i in batch]
        ano = [self.buffer[i][5] for i in batch]
        achi_g = [self.buffer[i][6] for i in batch]
        g_dif = [self.buffer[i][7] for i in batch]
        emb_s1 = [self.buffer[i][8] for i in batch]
        return s, a, r, s1, a1, ano, achi_g, g_dif, emb_s1
        # return emb_s1, g_dif

    def sample_batch_(self, ct):
        ct = min(ct, self.size)
        idx = [self.rg.choice(self.size) for _ in range(ct)]
        s = [self.buffer[i][0] for i in idx]
        a = [self.buffer[i][1] for i in idx]
        r = [self.buffer[i][2] for i in idx]
        s1 = [self.buffer[i][3] for i in idx]
