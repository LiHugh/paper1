...
class Critic(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, goal_dim):
        ...
        self.fc1 = nn.Linear(state_dim + action_dim + goal_dim, 400)
        self.fc1.weight.data = utls.fanin_init(self.fc1.weight.data.size())
        ...
        self.fca1 = nn.Linear(state_dim + action_dim + goal_dim, 400)
...
class DQNTrainer:
    ...
    def get_values2(self, nodes_attr, adj, action, goal):
        s = nodes_attr, adj
        a = Variable(torch.from_numpy(action.reshape((1,) + action.shape)).to(self.device))
        goal = Variable(torch.from_numpy(goal)).to(self.device)
        q1, q2 = self.actor_critic.critic_forward(np.array([s[0]], dtype=np.float32),
