...
from Final_env_with_goal_server import NetworkEnv
from goal import NetworkEnv2
from deepwalk import DeepWalk

seeds = []
rg2 = np.random.RandomState(10)
for i in range(500):
    seed = list(rg2.choice(1263, 5))
    seeds.append(seed)
...
def arg_parse():
    ...
    parser.add_argument('--logfile', dest='logfile', type=str, default='train_CHER.log',
                        help='Logging file')
    ...
    parser.add_argument('--cpu', dest='cpu', type=int, default=3,
...
batch_size2 = args.batch_size -20
batch_size1 = 20
...
if args.logdir is None:
    ...
    writer = SummaryWriter(os.path.join('runs_delete', args.logdir))
...
BUFF_SIZE2 = 80
replay_her = PriortizedReplay(BUFF_SIZE2, 10, beta=0.6)
    ...
    for epoch in range(1):
        for ep in range(NUM_EP):
            ...
            for stps in range(3):
                ...
                # print('#############################',n_state_embed/norm(n_state_embed))
                g_dif = num_infl - final_goal
                ...
                replay.add(s, actual_action_embed, reward, s1, s_embs[get_action(s1, s_embs, env.possible_actions)[0]],
                           actual_action, achi_g=num_infl, g_dif=g_dif, emb_s1=n_state_embed/norm(n_state_embed), td=np.abs(td))
                # print('add @@@@@@@@@@@@@@@@@@@@@@@@@@:', n_state_embed/norm(n_state_embed))
                s = s1
                if d:
                    break
            ...
            for stps in range(2):
                ...
                s, actual_action_embed, r1, s1, s_embs, actual_action, achi_g, pro_m, div_m = replay.sample_batch_(3)
                # print('s##############################',s)
                # print('#############################s_embs',s_embs)
                # print('div_m!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!',div_m)
                # diversity = [np.array(x, dtype=np.float32) for x in div_m]
                diversity = np.array(div_m)
                diversity = np.reshape(diversity, (-1,60))
                # print('diversity%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%', diversity)
                # print(diversity.dtype)
                ...
                print('feature matrix multiple result:', div)
                score = div.sum(axis=0) + proximity.T * balance
                print('diversity score......................', div.sum(axis=0))
                ...
                for i in range(2):
                    ...
                    print('new nnnnnnn sampled A^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^', mini_cher)
                    proximity[index] = np.NINF
                    print('proximity#################', proximity)
                    n_s = s[index]
                    n_s1 = s1[index]
                    ...
                    mini_s1.append(n_s1)
                    mini_a1.append(s_embs[index])
                s, actual_action_embed, r1, s1, s_embs, actual_action, achi_g, pro, div = replay.sample_batch(2)
                # print('new_______________________________________s', s)
                for idx in range(2):
                    # print('^^^^^^^^^^^^^^^^^^^^^^',idx)
                    # print('apend^^^^^^^^^^^^^^^^^^^^^^^^^^',s[idx])
                    mini_s.append(s[idx])
            ...
            logging.info('Episode: ' + str(ep) + 'Number of n_g achieved: ' + str() + 'number:' + str(final_num_infl))
            logging.debug('Chosen:'+ str(res))
            # logging.debug('Critic Loss: ' + str(acmodel.loss_critic))
            logging.debug('Final influenced number of nodes, num_infl:' + str(num_infl) + 'final_influenced number' + str(final_num_infl))
            rws.append(tot_r)
            ...
            if ep % save_every == 0:
                torch.save(acmodel, 'CHER_models/' + args.save_model + str(ep) + '.pth')
