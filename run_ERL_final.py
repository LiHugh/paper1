...
reset_prob = 0.2
epoch = 5
prior_scores = np.ones(num_models)
        ...
        for update in range(epoch):
            ...
            if (update==epoch-1):
                ...
                for i in range(mutate_num):
                    ...
                    idxs.append(index)
                    scores[index] = np.Inf
                # idxs = heapq.nsmallest(mutate_num, scores)
                # if write:
                #     writer.add_scalar('CriticLoss', acmodels[best_idx].loss_critic.clone().cpu().data.numpy(), n_iter)

                for i in idxs:
                    if rg3.rand() < reset_prob:
                        acmodels[i] = DQNTrainer(input_dim=input_dim, state_dim=action_dim, action_dim=action_dim, replayBuff=replay, lr=LR,
                                 use_cuda=use_cuda, gamma=args.gamma,
                                 eta=eta, gcn_num_layers=gcn_layers, num_pooling=num_pooling, assign_dim=assign_dim,
                                 assign_hidden_dim=assign_hidden_dim)
                    else:
                        acmodels[i] = acmodels[best_idx]
            ...
            if ep % save_every == 0:
                ...
                torch.save(acmodels[best_idx], 'models/' + args.save_model + str(ep) + '.pth')

            noise_param *= max(0.001, noise_decay_rate)
            # acmodel.eta = max(0.001, acmodel.eta * eta_decay)
            eta = max(0.001, eta * eta_decay)
