...
class Set2Set(nn.Module):
    def __init__(self, input_dim, hidden_dim, act_fn=nn.ReLU, num_layers=1, use_cuda=True):
        '''
        Args:
    ...
    def forward(self, embedding):
        ...
        hidden = (torch.zeros(self.num_layers, batch_size, self.lstm_output_dim).to(self.device),
                  torch.zeros(self.num_layers, batch_size, self.lstm_output_dim).to(self.device))

        q_star = torch.zeros(batch_size, 1, self.hidden_dim).to(self.device)
        for i in range(n):
