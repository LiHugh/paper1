...
syn = False
ratio = 5
os.makedirs("cher_models_form1", exist_ok=True)
os.makedirs("cher_logs_form1", exist_ok=True)


def arg_parse():
    parser = argparse.ArgumentParser(description='Influence Maxima Arguments')
    parser.add_argument('--logfile', dest='logfile', type=str, default='train_CHER_form2.log',
                        help='Logging file')
    ...
    parser.add_argument('--save_model', dest='save_model', type=str, default='CHER_sample_form1_',
    ...
    for epoch in range(1):
        for ep in range(NUM_EP):
            ...
            for stps in range(budget):
                ...
                episode_experience0.append((s, actual_action_embed, reward, s1, final_goal,
                                           s_embs[get_action(s1, s_embs, env.possible_actions)[0]], actual_action,
                                           np.abs(td), num_infl))
                s = s1
                if d:
            ...
            for stps in range(exp_size):
                ...
                s, actual_action_embed, r1, s1, final_goal, s_embs, actual_action, td, num_infl = episode_experience0[
                    stps]
                ...
                if HER:
                    for k in range(K_size):
                        _, _, _, _, _, _, _, _, g_n = episode_experience0[exp_size - 1]
                        final_num_infl = g_n
                        # n_g = int(g_n)
                        n_g = g_n
                        ...
                        replay_her.add(n_s, actual_action_embed, n_r, n_s1, s_embs, actual_action, td=td)
            ...
            if ep % save_every == 0:
                torch.save(acmodel, 'cher_models_form1/' + args.save_model + str(ep) + '.pth')
